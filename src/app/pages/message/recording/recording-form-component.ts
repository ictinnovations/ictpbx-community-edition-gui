...
export class AddRecordingComponent implements OnInit {
  ...
  constructor(private http: Http, private route: ActivatedRoute, private app_service: AppService,
    private recording_service: RecordingService, private router: Router) { }
  ...
  updateRecording(): void {
    this.recording_service.update_Recording(this.recording).then(() => {
      this.URL = `${this.app_service.apiUrlRecording}/${this.recording_id}/media`;
      if (this.file != null) { this.upload(); }
      this.router.navigate(['../../recording'], { relativeTo: this.route });
