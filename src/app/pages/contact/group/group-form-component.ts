  ...
  addGroup(): void {
    ...
    if (this.errorText.length === 0) {
      this.group_service.add_Group(this.group).then(response => {
        ...
        this.app_service.success_message = 'Group created succussfully';
        this.clearMsg();
      });
    }else{
      this.app_service.err_message = 'Error occurred while creating Group';
      this.clearMsg(); 
    }
  }
  ...
  updateGroup(): void {
    ...
    if (this.errorText.length === 0) {
      this.group_service.update_Group(this.group).then(() => {
        ...
        this.router.navigate(['../../group'], {relativeTo: this.route});
        this.app_service.success_message = 'Group_ID updated successfully';
        this.clearMsg();
      })
      .catch(this.handleError);
    }else{
      this.app_service.err_message = 'Error occurred while updating Group_ID';
      this.clearMsg();  
    }
  }
