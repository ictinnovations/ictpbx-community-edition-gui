...
export class AddTemplateComponent implements OnInit {
  ...
  constructor(private http: Http, private route: ActivatedRoute, private app_service: AppService,
    private template_service: TemplateService, private router: Router) { }
  ...
  updateTemplate(): void {
    this.template_service.update_Template(this.template).then(() => {
      this.URL = `${this.app_service.apiUrlTemplate}/${this.template_id}/media`;
      if (this.attachment != null) { this.upload(); }
      this.router.navigate(['../../template'], { relativeTo: this.route });
