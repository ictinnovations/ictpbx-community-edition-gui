...
export class DIDService {
  ...
  get_DIDList(filter = null, pageSize = null, pageIndex = null) {
    const headers = new Headers();
    this.app_service.createAuthorizationHeader(headers);
    const options = new RequestOptions({ headers: headers });
    var getUrl = `${this.app_service.apiUrlDid}`;
    if (filter) {
      getUrl = `${getUrl}?${filter}`;
    }
    if (pageSize) {
      getUrl = `${getUrl}?pageSize=${pageSize}&pageIndex=${pageIndex}`
    }
  ...
  gettotalnumberrows() {
    ...
    this.app_service.createAuthorizationHeader(headers);
    const options = new RequestOptions({ headers: headers });
    var getUrl = `${this.app_service.apiUrlDid}?totalrows`;
    return this.http.get(getUrl, options).toPromise()
  ...
  get_didList(tenant = 0): Promise<DID[]> {
    const headers = new Headers();
    this.app_service.createAuthorizationHeader(headers);
    let userUrl = this.app_service.apiUrlDid;
    if (tenant !== 0) {
      userUrl += `?tenant_id=${tenant}`;
    }
    const options = new RequestOptions({ headers: headers });
  ...
  assign_Imported_DID(account, user_id): Promise<DID> {
    const headers = new Headers();
    this.app_service.createAuthorizationHeader(headers);
    const options = new RequestOptions({ headers: headers });
    const body = JSON.stringify(account);
    const assignUrl = `${this.app_service.apiUrlAccounts}/${account}/users/${user_id}`;
    return this.http.put(assignUrl, body, options).toPromise().then(response => response.json() as DID)
      .catch(err => this.app_service.handleError(err));
  }
