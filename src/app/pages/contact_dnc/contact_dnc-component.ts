...
export class FormsContactDNCComponent implements OnInit {
  ...
  ngOnInit() {
    this.getContactDNClist(this.pageSize, this.pageIndex);
    this.gettotalrows();
    ...
    this.paginator.page.subscribe((event: PageEvent) => {
      ...
      if (event.previousPageIndex == event.pageIndex) {
        ...
        if (pageweight >= this.length) {
          ...
          this.getContactDNClist(event.pageSize, pageindex);
        }
      }
    });
  }
  ...
  gettotalrows() {
    this.contact_dnc_service.gettotalrows().then(data => {
      this.paginator.length = data;
    })
  }
  getContactDNClist(pageSize = null, pageIndex = null, filter = null, reget = 0) {
    this.filter = filter;
    this.contact_dnc_service.get_ContactDNCList(pageSize, pageIndex, filter).then(data => {
      if (filter) {
        this.length = data.length;
        this.paginator.length = data.length;
        this.paginator.pageIndex = 0;
        this.dncContactArray = data;
      } else {
        this.dncContactArray = this.dncContactArray.concat(data);
        this.length += data.length;
      }
      if (reget == 1) {
        this.dncContactArray = data;
      ...
      this.aContactDNC = new ContactDNCDataSource(new ContactDNCDatabase(this.dncContactArray), this.sort, this.paginator);
  ...
  deleteContactDNC_bk(contact_dnc): void {
    this.contact_dnc_service.delete_ContactDNC(contact_dnc)
      .then(response => {
      })
      .catch(this.handleError);
    this.getContactDNClist(10, 0, null, 1);
  }

  deleteContactDNC(contact_dnc): void {
    this.contact_dnc_service.delete_ContactDNC(contact_dnc)
      .then(() => {
        this.dncContactArray = this.dncContactArray.filter(
          item => item.contact_dnc_id !== contact_dnc
        );
        this.length = this.dncContactArray.length;

        this.aContactDNC = new ContactDNCDataSource(
          new ContactDNCDatabase(this.dncContactArray),
          this.sort,
          this.paginator
        );
