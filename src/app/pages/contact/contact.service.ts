...
export class ContactService {
  ...
  get_ContactList(pagesize = null, pageIndex = null, filter = null) {
    const headers = new Headers();
    this.app_service.createAuthorizationHeader(headers);
    const options = new RequestOptions({ headers: headers });
    var url = `${this.app_service.apiUrlContacts}`;
    if (filter) {
      url += `?${filter}`;
    }
    if (pagesize) {
      url = `${this.app_service.apiUrlContacts}?pagesize=${pagesize}&pageIndex=${pageIndex}`;
    }
  ...
  gettotalrows() {
    ...
    this.app_service.createAuthorizationHeader(headers);
    const options = new RequestOptions({ headers: headers });
    var url = `${this.app_service.apiUrlContacts}?totalrows`;
    return this.http.get(url, options).toPromise()
      .then(response => response.json()).catch(response => this.app_service.handleError(response));
  }
  ...
  update_disposition(contact: Contact): Promise<Contact> {
    const headers = new Headers();
    this.app_service.createAuthorizationHeader(headers);
    const options = new RequestOptions({ headers: headers });
    const body = JSON.stringify(contact);
    const url = `${this.app_service.apiUrlContacts}_disposition`;
    return this.http.post(url, body, options).toPromise()
      .then(response => response.json() as Contact)
      .catch(response => this.app_service.handleError(response));
  }

  get_contactdesposition(contact_id) {
    const headers = new Headers();
    this.app_service.createAuthorizationHeader(headers);
    const options = new RequestOptions({ headers: headers });
    const url = `${this.app_service.apiUrlContacts}/disposition/${contact_id}`;
    return this.http.get(url, options).toPromise()
